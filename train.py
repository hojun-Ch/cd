...
if __name__ == '__main__':
    ...
    if is_training_env:
        ...
        target_adversary_env = make_agent(name='adversary_env', env=venv, args=args, device=device)
    if args.ued_algo == 'domain_randomization' and args.use_plr and not args.use_reset_random_dr:
        adversary_env = make_agent(name='adversary_env', env=venv, args=args, device=device)
        adversary_env.random()
        target_adversary_env = make_agent(name='adversary_env', env=venv, args=args, device=device)
        target_adversary_env.random()
    # === Create runner ===
    plr_args = None
    if args.use_plr:
    ...
    train_runner = AdversarialRunner(
        ...
        target_adversary_env=target_adversary_env,
    ...
    for j in range(initial_update_count, num_updates):
        ...
        if log:
            # Eval
            test_stats = {}
            if evaluator is not None and (j % (args.test_interval // (args.repeat + 1)) == 0 or j == num_updates - 1):
        ...
        if checkpoint_idx != last_checkpoint_idx:
            is_last_update = \
                (j == num_updates - 1 and train_runner.student_grad_updates >= 1) or \
                j >= num_updates
            # is_last_update = \
        ...
        if save_screenshot:
            ...
            if args.env_name.startswith('BipedalWalker'):
                ...
                if args.use_editor and level_info:
                    ...
                    save_images(
                        images[:args.screenshot_batch_size], 
                        os.path.join(screenshot_dir, f'update{j * args.repeat}.png'),
                        normalize=True, channels_first=False)
                plt.close()
